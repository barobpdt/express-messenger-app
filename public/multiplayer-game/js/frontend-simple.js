...
class Player {
  constructor({ x, y, radius, color, username, character }) {
    this.x = x
    this.y = y
    this.radius = radius
    this.color = color
  ...
  draw() {
    ...
    if (this.character === 'circle') {
      c.arc(this.x, this.y, this.radius, 0, Math.PI * 2, false)
    } else if (this.character === 'square') {
      c.rect(this.x - this.radius, this.y - this.radius, this.radius * 2, this.radius * 2)
    } else if (this.character === 'triangle') {
      c.moveTo(this.x, this.y - this.radius)
      c.lineTo(this.x - this.radius, this.y + this.radius)
      c.lineTo(this.x + this.radius, this.y + this.radius)
      c.closePath()
    } else if (this.character === 'star') {
      const spikes = 5
      const outerRadius = this.radius
      const innerRadius = this.radius / 2
      let rot = Math.PI / 2 * 3
      let step = Math.PI / spikes
      c.moveTo(this.x, this.y - outerRadius)
      for (let i = 0; i < spikes; i++) {
        c.lineTo(this.x + Math.cos(rot) * outerRadius, this.y + Math.sin(rot) * outerRadius)
        rot += step
        c.lineTo(this.x + Math.cos(rot) * innerRadius, this.y + Math.sin(rot) * innerRadius)
        rot += step
      }
      c.closePath()
    }
