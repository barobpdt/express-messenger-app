 ...
 *
 * 사용법:
 *   node scripts/test-eval.js <파일경로> [--lines N] [--log <로그파일>] [--server <서버URL>] 
	...
	command=#[node test-eval.js --in "${in}" --out "${out}"]
...
logAppend(`@#>node-command: start=>👁  감시 시작: ${JSON.stringify(nodeInfo)} 파일:${nodeInfo.in}`);

if (nodeInfo.in < 1) {
	console.error('사용법: node scripts/test-eval.js --in [] [--lines N] [--out <로그파일>] [--server <서버URL>]');
	process.exit(1);
}
...
if (!fs.existsSync(targetPath)) {
	logAppend(`@#>node-command: error=>❌ 파일을 찾을 수 없습니다: ${targetPath}`);
	process.exit(1);
}
...
async function parseCommand(type, msg) {
	if(type=='quit') {
		logAppend(`@#>node-command: finish=>감시를 종료합니다.`);
		process.exit(0)
	}
	if( type=='nodeInfo') {
		logAppend(`@#>node-command: ${type}=>${JSON.stringify(nodeInfo)}`)
	}
	if( type=='eval') {
		eval(msg)
	}
...
function watchFile(filePath) {
	...
	const watcher = fs.watch(filePath, { encoding: 'utf8' }, (eventType)=>{
		...
		try {
			...
			// 파일이 삭제/재생성되는 경우
			logAppend(`@#>node-command: error=>⚠ 파일이 삭제되거나 교체되었습니다. 감시를 계속합니다...`);
			lastSize = 0;
			return;
		}
		...
		if (currentSize < lastSize) {
			...
			logAppend(`@#>node-command: error=>⚠ 파일이 초기화(truncate)되었습니다.`);
	...
	watcher.on('error', (err)=>{
		logAppend(`@#>node-command: error=>감시 오류: ${err.message}`);
	});
...
// ── 메인 ─────────────────────────────────────────────────────────────────────
logAppend(`@#>node-command: start=>👁  감시 시작: ${targetPath}`);
